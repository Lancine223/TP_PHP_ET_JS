...
<body>
        ...
        <?php
            ...
            if(isset($_POST['button'])){
                ...
                if(isset($matricule) && isset($nom) && isset($prenom) && $age && isset($date_naissance)&& isset($email)&& isset($tel)&& isset($img_des)&& isset($promotion)&& $annee_certification ){
                    // connexion à la base de donnée
                    // include_once "connexion.php";
                    // requête pour ajouter des apprenants
                    $req = mysqli_query($con, "INSERT INTO `apprenant` (`id`,`matricule`, `nom`, `prenom`, `age`, `date_naissance`, `email`, `tel`, `photo`, `promotion`, `annee_certification`) VALUES(Null, '$matcle', '$nom', '$prenom', '$age', '$date_naissance', '$email', '$tel', '$img_des', '$promotion', '$annee_certification')");
        ...
        <form class="br_input1" enctype="multipart/form-data" action="" method="POST">
            ...
            <div><a href="home.php" class="back_btn"><img src="retour.png" alt="">Retour</a><br>
                ...
                <div class="promo" id="id_p" name="id_p" style="display: none">
                    ...
                    <input class="button" type="submit" value="Ajouter" name="button_p">
                </div>    
            ...
            <input class="br_input" type="text" placeholder="Numéro Matricule" name="matricule" id="matcle">
            ...
            <input class="button" type="submit" value="Ajouter" name="button">
        </form>
</div>
        <!-- js link  -->
        <script src="script.js"></script>

        <!-- LA REQUET POUR LA PROMOTION  -->
        <!-- 
                    $req = mysqli_query($con, 'select * from promotion');

                    while($row = mysqli_fetch_assoc($req)){
                        if(isset($_POST['id_p']) && $row['id_p']== $_POST['id_p']){
                            echo '<option value="'.$row['nom_p'].'-'.$row['annee_p'].'" selected>'.$row['nom_p'].''
                            .'-'.$row['annee_p']. '</option>';
                        } else{
                            echo '<option value="'.$row['nom_p'].'-'.$row['annee_p'].'">'.$row['nom_p'].'-'.
                            $row['annee_p'].'</option>';
                        }
                    }
               -->
        <!-- FIN DE REQUET POUR LA PROMOTION  -->
</body>
</html>